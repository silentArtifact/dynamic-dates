...
/** Regex fragment matching any month name (full or abbreviated). */
...
// Pre-compiled regexes using shared MONTH_PATTERN
const NEEDS_YEAR_RE1 = new RegExp(
	`^(?:last|next)\\s+(${MONTH_PATTERN})\\s+\\d{1,2}(?:st|nd|rd|th)?$`, "i");
const NEEDS_YEAR_RE2 = new RegExp(
	`^(?:the\\s+)?(first|second|third|fourth|fifth|last)\\s+(sunday|monday|tuesday|wednesday|thursday|friday|saturday)\\s+(?:in|of)\\s+(${MONTH_PATTERN})\\s+\\d{2,4}$`, "i");
const NEEDS_YEAR_RE3 = new RegExp(
	`^(${MONTH_PATTERN})\\s+\\d{1,2}(?:st|nd|rd|th)?(?:,)?\\s*\\d{2,4}$`, "i");
...
// Pre-compiled regexes for phraseToMoment (using shared MONTH_PATTERN)
const MDY_RE = new RegExp(
	`^(${MONTH_PATTERN})\\s+(\\d{1,2})(?:st|nd|rd|th)?(?:,)?\\s*(\\d{2,4})$`, "i");
const LAST_MD_RE = new RegExp(
	`^last\\s+(${MONTH_PATTERN})\\s+(\\d{1,2}\\w*)$`, "i");
const NTH_WD_RE = new RegExp(
	`^(?:the\\s+)?(first|second|third|fourth|fifth|last)\\s+(sunday|monday|tuesday|wednesday|thursday|friday|saturday)\\s+(?:in|of)\\s+(${MONTH_PATTERN})(?:\\s+(\\d{2,4}))?`, "i");
const MD_RE = new RegExp(
	`^(${MONTH_PATTERN})\\s+(\\d{1,2}\\w*)$`, "i");
