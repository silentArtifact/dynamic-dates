...
/** Minimal interface for the plugin, avoids circular dependency with main.ts. */
export interface DDPluginInterface {
	settings: { acceptKey: "Enter" | "Tab"; noAliasWithShift: boolean };
	allPhrases(): string[];
...
export default class DDSuggest extends EditorSuggest<string> {
	plugin: DDPluginInterface;
	private _last: string[] = [];
	constructor(app: App, plugin: DDPluginInterface) {
		super(app);
		this.plugin = plugin;
	}
